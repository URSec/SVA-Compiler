...
; CHECK-NEXT: %1 = load i8, i8* %[[MASKED]]
; CHECK-NEXT: ret void
    %1 = load i8, i8* undef
    ret void
}
...
; CHECK-NEXT: %[[INT_MASKED:[[:alnum:]_.]+]] = or i64 %[[PTR]], %[[MASK]]
; CHECK-NEXT: %[[MASKED:[[:alnum:]_.]+]] = inttoptr i64 %[[INT_MASKED]] to i8*
; CHECK-NEXT: %1 = load atomic i8, i8* %[[MASKED]] seq_cst, align 8
; CHECK-NEXT: ret void
    %1 = load atomic i8, i8* undef seq_cst, align 8
    ret void
}
...
; CHECK-NEXT: %[[INT_MASKED:[[:alnum:]_.]+]] = or i64 %[[PTR]], %[[MASK]]
; CHECK-NEXT: %[[MASKED:[[:alnum:]_.]+]] = inttoptr i64 %[[INT_MASKED]] to i8*
; CHECK-NEXT: %1 = load volatile i8, i8* %[[MASKED]]
; CHECK-NEXT: ret void
    %1 = load volatile i8, i8* undef
    ret void
}
